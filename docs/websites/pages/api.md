...
- **接口名（`summary`）用中文**：动词开头、不超过 12 字、同模块内不重复（如「新增入库」「冲销流水」「导出采购申请表」）。
  它写在路由装饰器上（`@router.post(..., summary="新增入库")`），是 Apifox 里显示的接口名称。
  改接口时**必须一起写中文名**，否则 Apifox 里会退回 FastAPI 自动生成的英文名（`Get Settings` 之类）。
- **`operationId` 保持英文**：它是接口标识（MCP 的 `operation_call`、生成的前端方法名都依赖它），
  语义是「机读标识」而不是「展示名」，因此不做本地化。
